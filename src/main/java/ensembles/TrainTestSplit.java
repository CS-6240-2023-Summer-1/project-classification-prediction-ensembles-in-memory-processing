...
public class TrainTestSplit {
    public static void main(String[] args) {
        String csvFile = "/project-classification-prediction-ensembles-in-memory-processing/input/Final_Higgs_Data.csv"; // path of CSV file
    ...
    private static void saveDatasetToCSV(List<DatasetEntry> dataset, String csvFile, String delimiter) {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(csvFile))) {
            for (DatasetEntry entry : dataset) {
                String line = entry.getData() + delimiter + entry.getLabel();
                bw.write(line);
                bw.newLine();
            }
        } catch (IOException e) {
